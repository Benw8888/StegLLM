...
class PPOTrainer(BaseTrainer):
    ...
    def step(
        self,
        queries_encoder: List[torch.LongTensor],
        responses_encoder: List[torch.LongTensor],
        scores_encoder: List[torch.FloatTensor],
        queries_decoder=None,
        responses_decoder=None,
        scores_decoder=None,
    ):
        """
        ...
        Args:
            ...
            responses_encoder (List[`torch.LongTensor`]):
                List of tensors containing the encoded responses of shape (`response_length`)
            scores_encoder (List[`torch.FloatTensor`]):
                List of tensors containing the scores from encoding.
            queries_decoder (List[`torch.LongTensor`]):
                List of tensors containing the decoder token queries of shape (`query_length`)
            responses_decoder (List[`torch.LongTensor`]):
                List of tensors containing the encoded responses of shape (`response_length`)
            scores_decoder (List[`torch.FloatTensor`]):
                List of tensors containing the scores from decoding.
        ...
        queries_encoder, responses_encoder, scores_encoder = self._step_safety_checker(bs,
                                                                                       queries_encoder,
                                                                                       responses_encoder,
                                                                                       scores_encoder)

        queries_decoder, responses_decoder, scores_decoder = self._step_safety_checker(bs,
                                                                                       queries_decoder,
                                                                                       responses_decoder,
                                                                                       scores_decoder)

        scores = scores_encoder + scores_decoder
        ...
        model_inputs_encoder = self.prepare_model_inputs(queries_encoder, responses_encoder)
        model_inputs_decoder = self.prepare_model_inputs(queries_decoder, responses_decoder)

        for m in [model_inputs_encoder, model_inputs_decoder]:

            if self.is_distributed:
                pad_first = self.tokenizer.padding_side == "left"

                m["input_ids"] = self.accelerator.pad_across_processes(
                    m["input_ids"], dim=1, pad_index=self.tokenizer.pad_token_id, pad_first=pad_first
                )
                m["attention_mask"] = self.accelerator.pad_across_processes(
                    m["attention_mask"], dim=1, pad_index=0, pad_first=pad_first
                )
                if self.is_encoder_decoder:
                    m["decoder_input_ids"] = self.accelerator.pad_across_processes(
                        m["decoder_input_ids"],
                        dim=1,
                        pad_index=self.tokenizer.pad_token_id,
                        pad_first=pad_first,
                    )
                    m["decoder_attention_mask"] = self.accelerator.pad_across_processes(
                        m["decoder_attention_mask"], dim=1, pad_index=0, pad_first=pad_first
                    )

        model_inputs_names_encoder = list(model_inputs_encoder.keys())
        model_inputs_names_decoder = list(model_inputs_decoder.keys())

        with torch.no_grad():
            all_logprobs_encoder, _, values_encoder, masks_encoder = self.batched_forward_pass(self.model,
                                                                                               queries_encoder,
                                                                                               responses_encoder,
                                                                                               model_inputs_encoder)
            all_logprobs_decoder, _, values_decoder, masks_decoder = self.batched_forward_pass(self.model,
                                                                                               queries_decoder,
                                                                                               responses_decoder,
                                                                                               model_inputs_decoder)
        ...
        mini_batch_dict_encoder = {
            "queries": queries_encoder,
            "responses": responses_encoder,
            "logprobs": all_logprobs_encoder.to(torch.float32),
            "values": values_encoder.to(torch.float32),
            "rewards": rewards_encoder,
            "masks": masks_encoder,
        }
        mini_batch_dict_decoder = {
            "queries": queries_decoder,
            "responses": responses_decoder,
            "logprobs": all_logprobs_decoder.to(torch.float32),
            "values": values_decoder.to(torch.float32),
            "rewards": rewards_decoder,
            "masks": masks_decoder,
        }
